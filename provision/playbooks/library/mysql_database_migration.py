...
class Database:
    ...
    def create_migration_table(self):
        query = (
            "CREATE TABLE IF NOT EXISTS `%s` (`version` bigint(20) NOT NULL, `dirty` BOOLEAN DEFAULT 0, `name` varchar(255), PRIMARY KEY (`version`) );"
    ...
    def get_current_version(self):
        query = (
            "Select COALESCE(MAX(version),0) as starting_version from `%s` where dirty IS FALSE;"
            % self.migration_table
        )
        try:
            self.cursor.execute(query)
            return self.cursor.fetchone()["starting_version"]
        except mysql_driver.Error as e:
            self.module.fail_json(msg="Mysql Error: " + str(e))
        ...
        try:
            ...
            while row is not None:
                ...
                if self.migrations[pos][0] != row["version"]:
                    self.module.fail_json(
                        msg="version conflict version:%s not found having name %s"
                        % (
                            self.migrations[pos][0],
                            self.migrations[pos][1]["name"],
                        )
                    )
                ...
                if self.migrations[pos][1]["name"] != row["name"]:
                    self.module.fail_json(
                        msg="version conflict version:%s name:%s not matching with older name: %s"
                        % (
                            self.migrations[pos][0],
                            self.migrations[pos][1]["name"],
                            row["name"],
        ...
        for f in os.listdir(source):
            if os.path.isfile(os.path.join(source, f)):
                ...
                if version in migrations.keys():
                    migrations[version].update({"name": name, direction: f})
                else:
                    migrations[version] = {"name": name, direction: f}
    ...
    def migrate_up(self, up=None):
        ...
        for migrate in self.migrations:
            ...
            if self.starting_version >= migrate[0]:
                continue
            if (iteration >= up) and (up != 0):
                break
            try:
                if "up" not in migrate[1]:
                    self.module.fail_json(
                        msg="UP script not found for version: %s" % migrate[0]
                    )
                ...
                query = (
                    "Insert INTO `%s`( version, name ) VALUES (%s, '%s') ON DUPLICATE KEY UPDATE dirty=FALSE, name = '%s';"
                    % (
                        self.migration_table,
                        migrate[0],
                        migrate[1]["name"],
                ...
                self.db_connection.commit()
                iteration += 1
                self.final_version = self.get_current_version()  
                self.changed = True
                ...
                try:
                    if "down" in migrate[1]:
                        sql_file = os.path.join(
                            self.source_folder, migrate[1]["down"]
                        )
                    ...
                    self.db_connection.rollback()
                    query = (
                        "Insert INTO `%s`( version, name, dirty ) VALUES (%s, '%s', TRUE );"
                        % (
                            self.migration_table,
                            migrate[0],
                            migrate[1]["name"],
                    ...
                    self.module.fail_json(
                        msg="Mysql Error: Need manual intervention, version: %s got dirty   %s"
                        % (migrate[0], str(e))
                    )
                    self.cursor.execute(query)
                    self.db_connection.commit()
                ...
                self.module.fail_json(
                    msg="Mysql Error: version: %s -- %s" % (migrate[0], str(e))
                )      
    ...
    def migrate_down(self, down):       
        ...
        for migrate in reversed(self.migrations):
            ...
            if self.starting_version < migrate[0]:
                continue
            if (iteration >= down) and (down != 0):
                break
            try:
                if "down" not in migrate[1]:
                    self.module.fail_json(
                        msg="Down script not found for version: %s"
                        % migrate[0]
                    )
                ...
                query = (
                    "DELETE FROM `%s` where version = %s and name = '%s'"
                    % (self.migration_table, migrate[0], migrate[1]["name"])
                )
                self.cursor.execute(query)
                self.db_connection.commit()
                iteration += 1
                ...
                try:
                    if "up" in migrate[1]:
                        sql_file = os.path.join(
                            self.source_folder, migrate[1]["up"]
                        )
                    ...
                    self.db_connection.rollback()
                    query = (
                        "Insert INTO `%s`( version, name, dirty ) VALUES (%s, '%s', TRUE) ON DUPLICATE KEY UPDATE dirty=TRUE, name = '%s';"
                        % (
                            self.migration_table,
                            migrate[0],
                            migrate[1]["name"],
    ...
    def migrate_goto(self, goto):
        if self.starting_version < goto:
            self.migrate_up(goto - self.starting_version)       
        elif self.starting_version > goto:
            self.migrate_down(self.starting_version - goto)
        else:
            self.changed = False
...
def main():
    ...
    db = Database(
        module=module, migration_table=migration_table, source_folder=source
    )
    ...
    module.exit_json(
        changed=db.changed,
        msg={
            "starting_version": db.starting_version,
            "final_version": db.final_version,
        },
    )
